    ...
    export const TratamientosCRUD = ({
        ...
        const formFields = [
          { label: "Nombre", value: nombre, setter: setNombre, type: "text", placeholder: "Ej: Fotocromático", col: 6 },
        ...
        // --------------------------------- Titulos y acciones de los botones ---------------------------------	
          const title = isAdd ? 'Registrar Tratamiento' : isEdit ? 'Editar Tratamiento' : 'Confirmar Eliminación';
          const actionHandler = isAdd ? handleAdd : isEdit ? handleUpdate : handleDelete;
          const actionText = isAdd ? 'Registrar Tratamiento' : isEdit ? 'Actualizar Tratamiento' : 'Eliminar';
                  ...
                  <div className="modal-body crud-card-body" style={{ padding: '1.5rem' }}>
                    {isDelete ? (
                      <p className="text-center mb-0" style={{ fontSize: '1rem' }}>
                        ...
                        <strong style={{ color: '#343a40' }}>{selectedTratamientos?.nombre}</strong>?
                      </p>
                    ) : (
