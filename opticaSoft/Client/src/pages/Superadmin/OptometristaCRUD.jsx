...
export const OptometristaCRUD = ({
  ...
  const formFields = [
    { label: "Nombre", value: nombre, setter: setNombre, type: "text", placeholder: "Ej: John Snow", col: 6 },
    { label: "Cedula", value: noCedula, setter: setNoCedula, type: "text", placeholder: "Ej: 12541240", col: 6 },
  ];
  ...
  // --------------------------------- Titulos y acciones de los botones ---------------------------------	
    const title = isAdd ? 'Registrar Optometrista' : isEdit ? 'Editar Información de Optometrista' : 'Confirmar Eliminación';
    const actionHandler = isAdd ? handleAdd : isEdit ? handleUpdate : handleDelete;
    const actionText = isAdd ? 'Registrar Optometrista' : isEdit ? 'Actualizar Optometrista' : 'Eliminar';
            ...
            <div className="modal-body crud-card-body" style={{ padding: '1.5rem' }}>
              {isDelete ? (
                <p className="text-center mb-0" style={{ fontSize: '1rem' }}>
                  ¿Estás seguro de eliminar el Optometrista:<br />
                  <strong style={{ color: '#343a40' }}>{selectedOptometrista?.nombre}</strong>?
                </p>
              ) : (
