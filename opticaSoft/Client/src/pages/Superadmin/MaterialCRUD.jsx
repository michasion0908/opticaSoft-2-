...
export const MaterialCRUD = ({
  ...
  const formFields = [
    { label: "Nombre", value: nombre, setter: setNombre, type: "text", placeholder: "Ej: Policarbonato", col: 6 },
  ...
  const renderModal = (type) => {
    ...
    const title = isAdd ? 'Registrar Material' : isEdit ? 'Editar Material' : 'Confirmar Eliminación';
    const actionHandler = isAdd ? handleAdd : isEdit ? handleUpdate : handleDelete;
    const actionText = isAdd ? 'Registrar Material' : isEdit ? 'Actualizar Material' : 'Eliminar Material';
            ...
            <div className="modal-body crud-card-body" style={{ padding: '1.5rem' }}>
              {isDelete ? (
                <p className="text-center mb-0" style={{ fontSize: '1rem' }}>
                  ¿Estás seguro de eliminar el Material:<br />
                  <strong style={{ color: '#343a40' }}>{selectedMaterial?.nombre}</strong>?
                </p>
              ) : (
