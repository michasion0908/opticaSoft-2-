...
export const LentesContactoCRUD = ({
  ...
  selectedLentesContacto
}) => {
  //  --------------------------  Configuración común para los campos del formulario -----------------------------------------
  const formFields = [
    { label: "Marca", value: marca, setter: setMarca, type: "text", placeholder: "Ej: BAUSCH & LOMB", col: 6 },
    { label: "Modelo", value: modelo, setter: setModelo, type: "text", placeholder: "Ej: SofLens", col: 6 },
    { label: "Duración", value: duracion, setter: setDuracion, type: "text", placeholder: "Ej: 30 Días", col: 6 },
    { label: "Esfera", value: esfera, setter: setEsfera, type: "text", placeholder: "Ej: -8.00 a +6.00 ", col: 6 },
    { label: "Precio", value: precio, setter: setPrecio, type: "number", placeholder: "Ej: 1000.00", col: 6 },
  ];

  // Componente para renderizar cada campo del formulario (Realiza UPDATE Y CREATE)
  ...
  const renderModal = (type) => {
    ...
    // --------------------------------- Títulos y acciones de los botones ---------------------------------	
    const title = isAdd ? 'Registrar Lente de Contacto' : isEdit ? 'Editar Lente de Contacto' : 'Confirmar Eliminación';
    const actionHandler = isAdd ? handleAdd : isEdit ? handleUpdate : handleDelete;
    const actionText = isAdd ? 'Registrar Lente de Contacto' : isEdit ? 'Actualizar Lente de Contacto' : 'Eliminar Lente de Contacto';
            ...
            <div className="modal-body crud-card-body" style={{ padding: '1.5rem' }}>
              {isDelete ? (
                <p className="text-center mb-0" style={{ fontSize: '1rem' }}>
                  ¿Estás seguro de eliminar el Lente de Contacto:<br />
                  <strong style={{ color: '#343a40' }}>{selectedLentesContacto?.marca}</strong>?
                </p>
              ) : (
