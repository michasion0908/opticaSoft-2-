...
export const TipoLenteCRUD = ({
  ...
  const formFields = [
    { label: "Nombre", value: nombre, setter: setNombre, type: "text", placeholder: "Ej: Monofocal", col: 6 },
  ...
  const renderModal = (type) => {
    ...
    const title = isAdd ? 'Registrar Tipo de Lente' : isEdit ? 'Editar Tipo de Lente' : 'Confirmar Eliminación';
    const actionHandler = isAdd ? handleAdd : isEdit ? handleUpdate : handleDelete;
    const actionText = isAdd ? 'Registrar Tipo de Lente' : isEdit ? 'Actualizar Tipo de Lente' : 'Eliminar Tipo de Lente';
            ...
            <div className="modal-body crud-card-body" style={{ padding: '1.5rem' }}>
              {isDelete ? (
                <p className="text-center mb-0" style={{ fontSize: '1rem' }}>
                  ¿Estás seguro de eliminar el Tipo de Lente:<br />
                  <strong style={{ color: '#343a40' }}>{selectedTipoLente?.nombre}</strong>?
                </p>
              ) : (
